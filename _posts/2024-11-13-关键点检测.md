...
$$ \mathcal{L}_{C}^{t_k} = \Sigma_{j=1}^J \Sigma_p W(p) ||S_j^{t_k}(p) - S_j^{\ast}(p)||^2_2 $$
...
我们在每个stage都使用loss function，用来解决梯度消失的问题，因为每个stage结尾都有loss function，对梯度的值进行了补充。从而整体的的loss function就是：

$$ \mathcal{L} = \Sigma_{t=1}^{T_P} \mathcal{L}_{P}^{t} + \Sigma_{t=T_P + 1}^{T_P + T_C} \mathcal{L}_{C}^t $$

Confidence maps GT，$$S_j^{\ast}$$的计算方式是，先对于图片里的每个人$$k$$生成confidence map，$$S_{j,k}^{\ast}$$。$$x_{j,k} \in R^2$$表示人$$k$$的身体部分$$j$$在图中位置的ground truth。从而$$S_{j,k}^{\ast}的位置$$p \in R^2$$处的值就是：$$ S_{j,k}^{\ast}(p) = exp(-||p-x_{j,k}||^2_2 / \sigma^2) $$，其中$$\sigma$$控制峰的大小。从而整个图片（可能包含多个人）的ground truth就是：$$ S_j^{\ast}(p) = max_k S_{j,k}^{\ast}(p) $$
...
假设$$x_{j_1, k}$$和$$x_{j_2, k}$$是人$$k$$的身体部位$$j_1$$和$$j_2$$的ground truth，这两个部位组成了肢体$$c$$。对于肢体$$c$$上的一点$$p$$，$$L_{c,k}^{\ast}(p)$$是一个单位向量，从$$j_1$$指向$$j_2$$，对于其它的点，$$L_{c,k}^{\ast}(p)$$的值都是0。但肢体不仅是一条直线，而是一个具有一定宽度的长方形。具体来说：

$$v = (x_{j_2, k} - x_{j_1, l}) / ||x_{j_2, k} - x_{j_1, l}||$$是从$$x_{j_1, k}$$指向$$x_{j_2, k}$$的单位向量，$$v_{\perp}$$是垂直于$$v$$的一个单位向量，而对于任意的像素点$$p$$，如果其满足$$ 0 \leq v (p - x_{j_1,k}) \leq \lVert x_{j_2, k} - x_{j_1, k} \rVert$$ 和 $$\lvert v_{\perp} (p - x_{j_1,k}) \rvert \leq \sigma_l$$，
