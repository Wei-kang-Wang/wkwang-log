...
$$\Phi_{h,1}$$和$$\Phi_{h,2}$$的整体训练基于的是一个alternating optimization和self-training的过程：也就是优化$$\Phi_{h,1}$$和$$\Phi_{h,2}$$轮换进行，每一次training round结束，使用Hungarian algorithm做一次linear assignment，而且将这一次$$\Phi_{h,1}$$所得到的keypoints的location用于下一次更新$$\Phi_{h,1}$$时所用的假标签，也就是self-training方法。


**3.3. Learning an object landmark detector**

由3.2，我们获得了集合$$\mathcal X$$里所有图片的keypoints的位置，以及这些keypoints的对于cluster中心的归属关系，这一节的目标，就是训练一个在3.1里就提到的那个keypoint detector $$\Psi$$。为了达到这个目标，作者使用的方法是训练$$\Psi$$，对于每张图片$$x_j$$，输出$$M$$个heatmaps $$H_i$$，$$i=1,2,\cdots,M$$，每一个heatmap是通过stage1所得到的该keypoint的假标签（也就是keypoint的位置）为中心的一个Gaussian分布。
