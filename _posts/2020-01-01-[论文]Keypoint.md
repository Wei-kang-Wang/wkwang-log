...
Another version of this paper is: [From Keypoints to Object Landmarks via Self-Training Correspondence: A novel approach to Unsupervised Landmark Discovery](https://arxiv.org/pdf/2205.15895.pdf), which has been submitted to TPAMI.

**Abstract**

这篇文章所针对的问题是利用无监督的方式找到object的landmarks，也就是keypoints。作者基于两个新的角度，提出了和现有方法不同的方法：（1）self-training，从generic的keypoints出发，作者提出一个self-training的方法来学习一个能够自我改进的detector，使得其输出的结果越来越向object landmakrs靠拢。（2）correspondence，作者认为correspondence对于无监督的landmark discovery来说是一个关键的部分，并提出了一个优化策略，其在通过聚类来找到不同图片之间的object landmarks的correspondence和不利用labels来学习一个object landmark detector这两个任务之间来回切换优化。和之前的方法进行对比，这篇文章的方法对于视角或者角度变化较大的情况更为灵活。作者在很多复杂的数据集上，包括LS3D，BBCPose，Human3.6M等，验证了所提出方法的效果。


**1. Introduction**

object parts，也叫做keypoints或者landmarks，包含着物体在3D空间里的shape和空间结构信息，特别是对于可变性的objects，比如说人脸、身体和手。之前的研究工作主要集中于利用监督学习的方式对几类物体类别的landmarks学习其的detector，比如说facial（[Super-fan: Integrated facial landmark localization and super-resolution of real-world low resolution faces in arbitary poses with gans]()，[Style aggregated network for facial landmark detection]()，[Coarse-to-fine auto-encoder networks (cfan) for real=time face alignment]()）或者human body keypoints（[Human pose estimation via convolutional part heatmap regression]()，[Stacked hourglass networks for human pose estimation]()，[Simple baselines for human pose estimation and tracking]()），这些方法使用了上千张标注了keypoints位置的图片。但是对所有类别的物体的keypoints进行标注是不现实的，所以这篇文章关注的是如何用无监督学习方法来学习landmark detectors。

利用无监督学习的方式来进行keypoints检测似乎是个无法完成的任务。人类可以理解图片里object的类别，object的各个部分，角度变化，遮挡，以及自遮挡，从而可以正确的对图片里的objects的keypoints进行标注。因此，我们完全不知道该如何设计任务从而可以无监督的让network能够学习到object的landmarks。最近的工作主要集中于以下两种任务：（1）对于2D图片transformation的equivariance性质，包括[Discovery of latent 3d keypoints via end-to-end geometric reasoning]()，[Unsupervised learning of landmarks by descriptor vector exchange]()，[Unsupervised learning of object landmarks by factorized spatial embeddings]()；（2）图片生成，包括[Unsupervised learning of object landmarks through conditional image generation]()，[Object landmark discovery through unsupervised adaptation]()，[Unsupervised discovery of object landmarks as structural representations]()。这些方法，尽管对多种object类别都提供了keypoints识别的结果，但是也各有各的缺点，比如说检测到的keypoints没有明确的semantic meaning（也就是说，检测到的keypoints和人工标注的那些差别很大），或者说对于大的角度变化泛化效果不好（比如说3D rotation）。

>作者也说了，这篇文章提出的方法也不能保证就能找得到semantic的keypoints，但是倾向于找到semantic meaningful的keypoints。

在这篇文章里，作者主要针对以下两个新的角度设计模型：

* self-training：对于很多object类别来说，人工标注的keypoints的位置大多落在object表面的edges或者corners上，所以这些keypoints可以被一个generic keypoint detector所检测到。从generic，noisy的keypoints出发，作者提出一个self-training的方法来训练一个keypoint detector。
* correspondence via clustering：keypoint detector一个非常重要的性质是对于同一个种类的不同的图片能找到keypoints之间的correspondence。作者提出了一个学习框架，其在利用聚类进行correspondence recovery这个任务和object keypoints learning这个任务之间来回优化（alternatively）。据我们所知，将correspondence recovery加入object keypoints无监督学习的框架里这是首创。

和之前的方法相比，本文的方法可以学到更灵活的keypoints，也就是说其对于3D视角的改变也是鲁棒的。fig1给出了例子的说明。作者在很多数据集，包括LS3D，BBCPose，Human3.6M，上进行了大量的实验证明所提出方法的有效性。
...
只有一小部份工作是不通过手动标注的监督信号来学习物体的keypoints的。有一些方法通过在image transformation之中加入等价关系来解决这个问题（也就是说经过image transformation的两张图片的对应的keypoints是equivalent的）：[Discovery of lantent 3d keypoints via end-to-end geometric reasoning]()，[Unsupervised learning of landmarks by descriptor vector exchange]()，[Unsupervised learning of object landmarks by factorized spatial embeddings]()。这样的方法还被用在一些相关的其它任务中，包括learning object frames（[Unsupervised learning of object frames by dense equivalent image labelling]()），dense landmark detection（[Unsupervised learning of landmarks by descriptor vector exchange]()），对称可变形的物体的建模（[Modelling and unsupervised learning of symmetric deformable object categories]()），以及学习3D keypoints（[Discovery of lantent 3d keypoints via end-to-end geometric reasoning]()）。[Unsupervised learning of landmarks by descriptor vector exchange]()和这篇论文有些观点比较类似，认为keypoint detectors可以被认为是local image descriptors，基于这个假设，之前提到的经过image transformation的两张图片的keypoint是equivalent的这个约束条件就可以被拓展为descriptors也应该对于image transformation来说是不变的，是equivalent的。

>也就是说，前面那些方法使用了keypoints在经过transformation的图片之间是equivalent的约束，而引入了keypoint detector实际上就是local image descriptor这个想法之后，再结合两者，那么local image descriptor也应该是对于transformation是不变的。

其它的方法使用了generative models来做image reconstruction。在[Unsupervised part-based disentangling of object shape and appearance]()和[Unsupervised discovery of object landmarks as structural representations]()里，物体的keypoints是作为image auto-encoding的一个中间步骤被得到的。在[Unsupervised learning of object landmarks through conditional image generation]()和[Self-supervised learning of a facial attribute embedding from video]()里，任务被描述为一个有条件的image generation任务，其目标是从一张该图片的deformed版本以及所检测到的keypoints来重构原图片。在[Object landmark discovery through unsupervised adaptation]()里，任务被描述为从domain adaptation的角度出发，生成更多的稳定的keypoints。而上述所有的这些方法的一个固有的问题是，image generation任务和keypoints检测任务是没有关系的，这个代理任务会导致给出不那么informative的keypoints结果。

从方法的角度来看，这篇论文是第一个使用了self-training和利用聚类来检测keypoints之间的correspondence关系来做无监督keypoints检测的工作。而且，上述那些工作对于大的角度变换不鲁棒，而这篇文章提出的方法对其是鲁棒的。


*Self-training*

self-training表示的是一类方法，其模型所得出的预测作为假标签继续用于模型训练。通常情况下，预测结果会被转换为严格的标签，并且仅仅会在置信度非常高的时候被保留下来。同一张图片经过不同的transformation之后被输出网络，其得到的标签应该是一样的。这样的知识也被用于获取更加准确的假标签。大多数的self-training方法聚焦于image classification任务。和本文方法更加相似的方法是那些针对unsupervised segmentation（[Boxsup: Exploiting bounding boxes to supervised convolutional networks for semantic segmentation]()，[Simple does it: Weakly supervised instance and semantic segmentation]()），foreground-background segmentation（[Video segmentation by non-local consensus voting]()，[Multiple frames matching for object discovery in video]()），salience object detection（[Supervised by fusion: Towards unsupervised learning of deep salient object detector]()）的文章。据作者所知，并没有其它使用self-training方法来做无监督物体keypoints检测任务的工作。


*Deep Clustering*

本文的方法也可以被认为和通过一个聚类的代理任务来做representation learning这样的方法（[Deep clustering for unsupervised learning of visual features]()，[Unsupervised visual representation learning by graph-based consistent constraints]()，[Learning deep parsimonious representations]()，[Boosting self-supervised learning via knowledge transfer]()，[Unsupervised deep embedding for clustering analysis]()，[Joint unsupervised learning of deep representations and image clusters]()）很像。通常来说，这些方法将图片归为不同的clusters，然后训练一个CNN来判别图片属于哪一类cluster或者使用cluster作为假标签来训练。据作者所知，使用聚类来进行物体keypoints的correspondence的学习，这样的方法是本文第一个提出来的。
...
$$\mathcal X = \lbrace x \in \mathbb R^{W \times H \times 3} \rbrace$$表示的是同一个类别物体的$$N$$张图片组成的集合（比如说human faces，human bodies等）。在$$\mathcal X$$上运行了一个generic的keypoint detector之后，将训练集$$\mathcal X$$扩展为了$$\lbrace x_j, \lbrace P_i^j \rbrace_{i=1}^{N_j} \rbrace，其中$$P_i^j \in \mathbb R^2$$表示的是一个keypoint，而$$N_j$$是图片$$x_j$$里所检测到的keypoints的个数。对于图片$$x_j$$所检测到的这些keypoints，$$P^j$$，其没有任何的顺序信息和对应关系。而且我们的图片里都只有一个object，所以说检测到的$$P^j$$都是一个物体的，而不是多个物体的，但仍有可能会有一些不是物体的keypoints被检测出来，可能是背景的keypoints。

我们的目的是训练一个网络 $$\Phi: \mathcal X \rightarrow \mathcal Y$$，其中$$Y \in \mathbb R^{H_o \times W_o \times K}$$表示的是$$K$$个想要检测的keypoints的heatmaps。注意到$$\mathcal Y$$的结构说明keypoints的顺序和对应关系都是需要被网络所预测出来的。同样注意到，$$K$$是所要检测到的物体的keypoints的个数，它并不是一个超参数，也是被网络所预测的。实验表明，网络所预测的$$K$$值和数据集以及初始化时候的generic detector所检测到的keypoints的个数有关，也就是和每张图片$$x_j$$的$$N_j$$有关。

对网络的训练分为两个stages。第一个stage目标是建立keypoints之间的correspondence，recover丢失的keypoints以及过滤掉无关的背景keypoints。第二个stage是将第一个stage的输出用来训练最终的keypoints detector。在这个时候，前一个stage里检测到的相似的keypoints被聚类到了一起形成最终的输出。


**3.2. Recovering  correspondence**

模型是一个neural network $$\Phi$$，其有一个共享的backbone $$\Phi_b$$，以及两个heads $$\Phi_{h,1}$$和$$\Phi_{h,2}$$，用来处理不一样的任务：

**A detector head $$\Phi_{h,1}$$**：$$\mathcal X \rightarrow \mathcal Z$$，其中$$\mathcal Z \in \mathbb R^{H_o \times W_o \times 1}$$是一个单通道的confidence maps。$$\Phi_{h,1}$$用来检测图片里所有的keypoints，而且它们是没有顺序和对应关系的，也就是说不需要对这些检测到的keypoints进行区分，所以说一个heatmap就够了。$$\Phi_{h,1}$$的主要目的是recover丢失了的keypoints。

这个detector使用self-training来进行训练，从而逐步改进自己的输出。在每个训练循环$$t$$，其使用训练循环$$t-1$$和$$t-2$$时候得到的假的keypoints的位置作为假标签来学习$$\Phi_{h,1}$$。这些假的keypoints的位置就是将模型在上两个循环的输出，很简单的去掉了那些不靠近任何一个cluster中心的那些keypoints位置。这个detector $$\Phi_{h,1}$$是用一个MSE loss来进行训练的：$$L_d = \lVert H(x_j) - \Phi_{h,1}(x_j) \rVert^2$$，其中图片$$x_j$$的所有的keypoints的集合$$\lbrace P_i^j \rbrace_{i=1}^{N_j}$$是用一个heatmap $$H$$表示的，其就是在每个keypoint位置上加上了一个高斯分布所得到的。

本文的self-training方法进一步证实了最近有关self-training的研究所得出来的一个结论，也就是使用self-training方法进行训练，学习能力足够的（over-parameterized）神经网络倾向于先学习没有噪声的那些类别，之后才会去学习有噪声的数据从而进一步减少训练误差。作者观察到在使用self-training方法学习object keypoints的时候有一个现象：在很多张图片里都会出现的真实的keypoints会有一个很高的detection置信度，而那些背景的keypoints因为并不会经常出现而且会变化，所以会被过滤掉。证实了这个结论。


**A feature extractor head $$\Phi_{h,2}$$**: \mathcal X \rightarrow \mathcal D$$，其中$$\mathcal D \in \mathbb R^{H_o \times W_o \times d}$$，而这个head是用来recover不同图片的keypoints之间的correspondence的。对于每个keypoint $$P_i^j$$，$$\Phi_{h,2}$$都会学习一个$$d$$维的feature descriptor $$f_i^j$$。这个descriptor是用来将检测到的keypoints聚类为$$M$$个clusters而用的，每个cluster表示一个物体的真实的keypoint或者同一个真实的keypoint的不同角度或者appearance。文章的方法使用K-means来计算cluster的中心以及每个keypoints该归类于哪个cluster。而且，每个keypoint能且只能属于一个cluster。从而，一个改进版的K-means问题就如下所示：

$$ min_{C \in \mathbb R^{d \times M}} \frac{1}{N} \Sigma_{i=1}^N \Sigma_{j=1}^{N_j} min_{y_i^j \in \lbrace 0, 1 \rbrace^M} \lVert f_i^j - C y_i^j \rVert_2^2$$

$$s.t., \pmb 1_M^T y_i^j = 1, \lVert \Sigma_j y_i^j \rVert_0 = N_j$$

在实践中，文章先使用传统的K-means算法来非常高效的找到$$M$$个cluster中心，也就是上述的$$C$$，然后再使用Hungarian algorithm来做linear assignment，来找到每张图片的keypoints和cluster之间的对应关系，从而确保每个keypoint只被归属于一个cluster（Hungarian algorithm是在每个training round结束的时候进行计算的，而不属于网络训练的一部分）。而且，和任何一个cluster中心都不靠近的keypoints就会被过滤掉。

作者注意到，将$$M$$设置为远大于$$K$$是很有必要的（$$K$$是预估的图片里的keypoints的个数）。这样可以使得文章的方法对于每个keypoint可以recover出来多个clusters，这对于处理因为角度变化而导致的大的appearance变化是非常有必要的。这个设置也使得本文的方法区分与之前的那些工作，他们并没有考虑到大的out-of-plane的rotation。而将对应于同一个keypoint的clusters进行合并则会在下一个stage被解决。

>实际上到这一步为止，$$\Phi_{h,2}$$都还是online计算的，也就是说上述过程并不是加入训练之中的，而下面的contrastive loss才是真正用来训练$$\Phi_{h,2}$$的。

现在我们已经有了keypoint-to-cluster assignments，下一步就可以在图片之间recover keypoints之间的correspondences，并且对于成对输入的图片使用图片的keypoints之间的这些correspondences来训练$$\Phi_{h,2}$$了。我们希望当$$y_i^j = y_{i^'}^{j^'}$$时，在点$$P_i^j$$处获得的features和在点$$P_{i^'}^{j^'}$$处获得的features尽可能的相似，而在其它情况下，尽可能的不相似。也就是说，不同图片里归属于同一个cluster的keypoint的features要尽可能的相似，其余情况尽可能不相似。作者将这个目标表示为了一个contrastive loss：

$$L_c(i,i^', j, j^') = \pmb 1_{\left[y_i^j = y_{i^'}^{j^'} \right]} \lVert f_i^j - f_{i^'}^{j^'} \rVert^2 + \pmb 1_{\left[y_i^j \neq y_{i^'}^{j^'} \right]} max (0, m - \lVert f_i^j - f_{i^'}^{j^'} \rVert^2)$$

其中$$\pmb 1$$是indicator函数，而$$m$$是margin，是个超参数。作者既在不同的图片之间找到这些点对，也在同一张图片的不同的数据增强获得的图片之间找到这些点对。

$$\Phi_{h,1}$$和$$\Phi_{h,2}$$的整体训练基于的是一个alternating optimization和self-training的过程：每一次training round结束（$$\Phi$$训练更新一次，包括backbone和两个heads），使用Hungarian algorithm做一次linear assignment，计算出$$y_i^j$$和cluster中心。而且将这一次$$\Phi_{h,1}$$所得到的keypoints的location用于下一次更新$$\Phi_{h,1}$$时所用的假标签，也就是self-training方法。


**3.3. Learning an object landmark detector**

由3.2，我们获得了集合$$\mathcal X$$里所有图片的keypoints的位置，以及这些keypoints的对于cluster中心的归属关系，这一节的目标，就是基于3.2的训练基础上，更进一步的训练这个网络。为了达到这个目标，作者使用的方法是训练$$\Psi$$，对于每张图片$$x_j$$，输出$$M$$个heatmaps $$H_i$$，$$i=1,2,\cdots,M$$，每一个heatmap是通过stage1所得到的该keypoint的假标签（也就是keypoint的位置）为中心的一个Gaussian分布。

正如3.2里所说的，很多clusters实际上对应的是同一个keypoint。因此，在这个stage里，还需要进行progressive cluster merging操作。为了实现这个目标，作者凭借$$\Psi$$的结构优势（一个hourglass network）设置了一个简单的算法，将appearance和location相关的信息都考虑了进来：
...
### 11. [Unsupervised Discovery of Object Landmarks as Structural Representataions]()
...
$$\bar \pmb D_k(u,v) = (1 / WH) \mathcal N((u,v); (x_k, y_k), \sigma_{det} \mathcal I) \tag{6}$$
...
对于每个landmark来说，作者使用一个average pooling来获取这个landmark的local feature descriptor，其中这个average pooling的权重是由一个中心在这个landmark点的soft mask构成的。具体来说，我们将公式6定义的$$\bar \pmb D_k$$作为这个soft mask（公式6定义的这个分布实际上是detection confidence map的一个高斯分布的近似）。之后，一个可学习的linear operator（实际上就是几层MLP）被加在每个landmark用上述方式所获取的features上从而为每个landmark学习到一个独立的feature descriptor（注意到每个landmark都有一个自己的linear operator，不是共享的）。
...
$$\pmb f_k = \pmb W_k \sum_{v=1}^H \sum_{u=1}^W (\bar \pmb D_k(u,v) \cdot \pmb F(u,v)) \in \mathbb R^{C} \tag{10}$$

其中$$C < S$$。每个landmark独有的这个linear operator使得每个landmark descriptor可以编码独有的信息。公式10也可以被用来获取一个背景的descriptor。使用之前所说的方法利用一个高斯分布来近似背景的confidence map是不合理的，所以直接令$$\bar \pmb D_{K+1} = \pmb D_{K+1} / \zeta_{K+1}$$。注意到$$\pmb f_k$$对于feature map $$\pmb F$$和detection confidence map $$\pmb D_k$$来说都是可微的。
