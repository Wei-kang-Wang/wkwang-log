...
$$\mathcal X = \lbrace x \in \mathbb R^{W \times H \times 3} \rbrace$$表示的是同一个类别的物体的$$N$$张图片组成的集合（比如说人脸，人的身体等）。在$$\mathcal X$$上运行了一个generic的keypoint detector之后，将训练集$$\mathcal X$$扩展为了$$\lbrace x_j, \lbrace P_i^j \rbrace_{i=1}^{N_j} \rbrace，其中$$P_i^j \in \mathbb R^2$$表示的是一个keypoint，而$$N_j$$是图片$$x_j$$里所检测到的keypoints的个数。对于图片$$x_j$$所检测到的这些keypoints，$$P^j$$，其没有任何的顺序信息和对应关系。而且我们的图片里都只有一个object，所以说检测到的$$P^j$$都是一个物体的，而不是多个物体的，但仍有可能会有一些不是物体的keypoints被检测出来，这是因为背景的原因，可能是背景的keypoints。

如果只使用$$\mathcal X$$，我们的目的是训练一个网络 $$\Phi: \mathcal X \rightarrow \mathcal Y$$，其中$$Y \in \mathbb R^{H_o \times W_o \times K}$$表示的是$$K$$个想要检测的keypoints的heatmaps。注意到$$\mathcal Y$$的结构说明keypoints的顺序和对应关系都是需要被网络所预测出来的。同样注意到，$$K$$是索要检测到的物体的keypoints的个数，它并不是一个超参数，也是被网络所预测的。实验表明，预测的$$K$$值和数据集，以及初始化时候的generic detector所检测到的keypoints的个数有关，也就是和每张图片$$x_j$$的$$N_j$$有关。

作者将方法分为两个stages来进行。第一个stage目标是建立keypoints之间的correspondence，recover丢失的keypoints以及过滤掉无关的背景keypoints。第二个stage是将第一个stage的输出用来训练一个keypoints detector。在这个时候，相似的keypoints就被聚类到了一起。
...
**A detector head $$\Phi_{h,1}$$**：$$\mathcal X \rightarrow \mathcal Z$$，其中$$\mathcal Z \in \mathbb R^{H_o \times W_o \times 1}$$是一个单通道的confidence maps。$$\Phi_{h,1}$$用来检测所有的物体的keypoints，而且它们是没有顺序的，也不需要检测不同图片的keypoints之间的correspondence，也就是说不需要对这些检测到的keypoints进行区分，所以说一个heatmap就够了。$$\Phi_{h,1}$$的主要目的是recover丢失了的物体的keypoints。

这个detector使用self-training来进行训练，从而逐步改进自己的输出。在每个训练循环$$t$$，其使用训练循环$$t-1$$和$$t-2$$时候得到的假的keypoints的位置作为标签来学习$$\Phi_{h,1}$$。这些假的keypoints的位置就是很简单的去掉了那些不靠近任何一个cluster中心的那些点之后的模型的输出。这个detector $$\Phi_{h,1}$$是用一个MSE loss来进行训练的，$$L_d = \lVert H(x_j) - \Phi_{h,1}(x_j) \rVert^2$$，其中图片$$x_j$$的所有的keypoints的集合$$\lbrace P_i^j \rbrace_{i=1}^{N_j}$$是用一个heatmap $$H$$表示的，其就是在每个keypoint位置上加上了一个高斯分布所得到的。

本文的self-training方法进一步证实了最近有关研究得出来的一个结论，也就是学习能力足够的（over-parameterized）神经网络倾向于先学习没有噪声的那些类别，之后才会去学习有噪声的数据从而进一步减少训练误差。作者观察到在使用self-training方法学习object keypoints的时候有一个现象：在很多张图片里都会出现的真实的keypoints会有一个很高的detection置信度，而那些背景的keypoints因为并不会经常出现而且会变化，所以会被过滤掉。


**A feature extractor head $$\Phi_{h,2}$$**: \mathcal X \rightarrow \mathcal D$$，其中$$\mathcal D \in \mathbb R^{H_o \times W_o \times d}$$，而这个head是用来recover不同图片的keypoints之间的correspondence的。对于每个keypoint $$P_i^j$$，$$\Phi_{h,2}$$都会学习一个$$d$$维的feature descriptor $$f_i^j$$。这个descriptor
