...
> 本文的实验是在一个car dataset上做的，该dataset背景复杂，且可能含有多个cars，但car本身是rigid objects，所以该数据集有它的难度，但在rigidity上变化较小。而且具体操作的时候，每次是将每张照片按照ground truth的bounding box裁剪掉多余的背景之后再做为输入，也就是每次只有一个center positioned的car，如下流程图所示。
