...
* $$H_{out} = \lfloor \frac{H_{in} + 2 \times padding \left[0 \right] - dilation \left[0 \right] \times (kernel_size \left[0 \right] - 1) - 1}{stride \left[0 \right]} + 1 \rfloor$$
* $$W_{out} = \lfloor \frac{W_{in} + 2 \times padding \left[0 \right] - dilation \left[0 \right] \times (kernel_size \left[0 \right] - 1) - 1}{stride \left[0 \right]} + 1 \rfloor$$
...
* 输入：$$(N, C_{in}, H_{in}, W_{in})$$或者$$(C_{in}, H_{in}, W_{in})$$
* 输出：$$(N, C_{out}, H_{out}, W_{out})$$或者$$(C_{out}, H_{out}, W_{out})$$，其中：
* $$H_{out} = (H_{in} − 1) \times stride \left[0 \right] − 2 \times padding \left[ 0 \right] + dilation \left[ 0 \right] \times (kernel_size \left[ 0 \right]−1) + output_padding \left[ 0 \right] + 1$$
* $$W_{out} = (W_{in} − 1) \times stride \left[0 \right] − 2 \times padding \left[ 0 \right] + dilation \left[ 0 \right] \times (kernel_size \left[ 0 \right]−1) + output_padding \left[ 0 \right] + 1$$
