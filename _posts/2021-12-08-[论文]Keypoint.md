...
现在在Internet上有非常多的3D shapes，所以这样一个任务：给用户提供intuitive和简单的interface，让他们可以在保留关键shape性质的情况下对3D object做semantically manipulating，在AI辅助的3D content creation领域有着非常多的研究。在这篇文章里，我们提出为interative editing自动检测intuitive和semantically有意义的control points，让对于每个object类别的保留细节的shape deformation成为可能。
