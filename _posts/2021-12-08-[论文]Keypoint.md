...
](https://openaccess.thecvf.com/content_cvpr_2017/papers/Qi_PointNet_Deep_Learning_CVPR_2017_paper.pdf)。但是我们使用的是单个输入，从而避免了Siamese network很复杂的训练过程。fig 2展示了网络结构，它的输入是一个shape $$S_i$$，且在$$SO_2$$里是misaligned的。
...
*Node branch* 这个branch预测了一个稀疏的nodes集合，它们是潜在的category-specific keypoints，但是并没有被排序。
