...
其中$$\sigma_{ij} = \frac{\sigma_i + \sigma_j^{'}}{2}$$，$$d_{ij} = min_{Q_j^{'} \in Q^{'}} \left| \left| Q_i - Q_j^{'} \right| \right| \geq 0$$。

$$p(d_{ij} \left| \sigma_{ij})$$是一个合规的概率分布。$$d_{ij}$$越小，那么proposal keypoints $$Q_i$$和$$Q_j^{'}$$是高度repeatable以及定位准确的keypoints的概率就越高。
...
其中$$\sigma_ji = \frac{\sigma_i + \sigma_j^{'}}{2}$$，$$d_ji = min_{Q_i \in Q} || Q_i - Q_j^{'}|| \geq 0$$。
